...
const GameControls = ({ onControl, sensitivity }: GameControlsProps) => {
  ...
  const press = useCallback((key: "left" | "right" | "up" | "down") => {
    pressedRef.current[key] = true;
    emitState();
  }, [emitState]);

  const release = useCallback((key: "left" | "right" | "up" | "down") => {
  ...
  return (
    <>
      {/* Up & Down arrows - right side, vertically stacked */}
      <div
        className="fixed right-3 z-50 flex flex-col items-center gap-2"
        style={{ bottom: 160 }}
      >
        <button
          className={btnBase}
      ...
      {/* Left & Right arrows - right side, below up/down */}
      <div
