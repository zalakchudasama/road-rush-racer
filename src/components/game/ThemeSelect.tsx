...
const ThemeSelect = ({ onSelect, onGarage }: Props) => (
  ...
  >
    ...
    >
      ...
      <p className="text-muted-foreground text-sm mb-4">Choose your road</p>
        ...
        {themeList.map((id) => {
          ...
          return (
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              onClick={() => onSelect(id)}
              className="flex flex-col items-center gap-1 px-4 py-4 rounded-xl border-2 border-border bg-card hover:border-primary transition-colors"
            >
